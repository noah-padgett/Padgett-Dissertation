...
# Model 6: Full IFA with Misclassification
mydata <- list(
...
jags.model <- function(){
  ...
  for(n in 1:N){
    for(i in 1:nit){
      ...
      # compute ELRT 
      ELRT[n,i] = (exp(mu.rt[n,i]))/abs(dev[n,i])
...
# save fitted model
save(fit6,file = paste0(outputdir,"model6_fit.RData"))

#load(paste0(outputdir,"model6_fit.RData"))
# print results
fit6$BUGSoutput$summary[
  !(rownames(fit6$BUGSoutput$summary) %like% "eta" |  
      rownames(fit6$BUGSoutput$summary) %like% "gamma"|  
      rownames(fit6$BUGSoutput$summary) %like% "ELRT"), ]
write.table(fit6$BUGSoutput$summary, file="output/posterior_summaries_model6.txt", sep = "\t")
# extract posteriors for all chains
jags.mcmc <- as.mcmc(fit6)
fit6.mcmc <- data.frame(as.matrix(jags.mcmc, chains=T, iters = T))
colnames(fit6.mcmc) <- c("chain", "iter", colnames(as.data.frame(jags.mcmc[[1]])))

# Posterior Summary
# Density
bayesplot::mcmc_areas(fit6.mcmc,regex_pars = "tau", prob = 0.8)
bayesplot::mcmc_areas(fit6.mcmc,regex_pars = "lambda", prob = 0.8)
bayesplot::mcmc_areas(fit6.mcmc,regex_pars = "theta", prob = 0.8)
bayesplot::mcmc_areas(fit6.mcmc,regex_pars = "rho", prob = 0.8)
bayesplot::mcmc_areas(fit6.mcmc,regex_pars = "icept", prob = 0.8)
bayesplot::mcmc_areas(fit6.mcmc,regex_pars = "sigma.t", prob = 0.8)
bayesplot::mcmc_areas(fit6.mcmc,regex_pars = "cor.ts", prob = 0.8)

# posterior autocorrelation
bayesplot::mcmc_acf(fit6.mcmc,regex_pars = "tau")
bayesplot::mcmc_acf(fit6.mcmc,regex_pars = "lambda")
bayesplot::mcmc_acf(fit6.mcmc,regex_pars = "theta")
bayesplot::mcmc_acf(fit6.mcmc,regex_pars = "rho")
bayesplot::mcmc_acf(fit6.mcmc,regex_pars = "icept")
bayesplot::mcmc_acf(fit6.mcmc,regex_pars = "sigma.t")
bayesplot::mcmc_acf(fit6.mcmc,regex_pars = "cor.ts")

# posterior traceplots
bayesplot::mcmc_trace(fit6.mcmc,regex_pars = "theta")
bayesplot::mcmc_trace(fit6.mcmc,regex_pars = "lambda")
bayesplot::mcmc_trace(fit6.mcmc,regex_pars = "tau")
bayesplot::mcmc_trace(fit6.mcmc,regex_pars = "rho")
bayesplot::mcmc_trace(fit6.mcmc,regex_pars = "icept")
bayesplot::mcmc_trace(fit6.mcmc,regex_pars = "sigma.t")
bayesplot::mcmc_trace(fit6.mcmc,regex_pars = "cor.ts")

# posterior GRB convergence
fit6.mcmc.ggs <- ggmcmc::ggs(jags.mcmc)
ggmcmc::ggs_grb(fit6.mcmc.ggs, family="theta")
ggmcmc::ggs_grb(fit6.mcmc.ggs, family="tau")
ggmcmc::ggs_grb(fit6.mcmc.ggs, family="lambda")
ggmcmc::ggs_grb(fit6.mcmc.ggs, family="rho")
ggmcmc::ggs_grb(fit6.mcmc.ggs, family="icept")
ggmcmc::ggs_grb(fit6.mcmc.ggs, family="sigma.t")
ggmcmc::ggs_grb(fit6.mcmc.ggs, family="cor.ts")


# Posterior Predictive Check
Niter <- 200
fit6$model$recompile()
fit6.extra <- rjags::jags.samples(fit6$model, variable.names = "omega", n.iter = Niter)
N <- mydata$N
nit <- C <- 5
n <- i <- iter <- 1
...
for(iter in 1:(Niter*2)){
  ...
  for(i in 1:nit){
    for(n in 1:N){
      ...
      y[n,i] <- sample(1:5, 1, prob = fit6.extra$omega[n,i, 1:5, iter.i, chain])
    }
...
p <- plot.ppc %>%
  ...
  theme_classic()
p
ggsave(paste0(outputdir,"fig3-model6-ppc.png"),plot = p, width=7, height=5, units="in")
ggsave(paste0(outputdir,"fig3-model6-ppc.pdf"),plot = p, width=7, height=5, units="in")
...
for(i in 1:nit){
  ...
  for(c in 1:C){
    for(cc in 1:C){
      gamma[c,cc] <- mean(colMeans(fit6.mcmc)[paste0("gamma[",1:N,",",i,",",c,",",cc,"]")])
    }
  }
  gammaList[[i]] <- gamma
}
...
plot.dat <- t(matrix(colMeans(fit6.mcmc)[colnames(fit6.mcmc) %like% "gamma"], nrow=125))
...
p  
ggsave(paste0(outputdir,"model6-gamma-dist.pdf"),plot = p, width=10, height=10, units="in")
...
p  
ggsave(paste0(outputdir,"model6-gamma-dist-v2.pdf"),plot = p, width=10, height=10, units="in")
# Evaluate relationship between Eta-item distance and RT
lrt <- pseudo_log(as.matrix(sdat_full[,colnames(sdat_full) %like% "RT_"]))
person <- colMeans(fit6.mcmc)[paste0("eta[",1:N,"]")]
plot.dat <- data.frame(
  id = 1:N,
  person,
  item = paste0("item_",1),
  PIdiff = person - mean(colMeans(fit6.mcmc)[paste0("tau[1,",1:4,"]")]),
  lrt = lrt[,1]
)
for(i in 2:5){
  p1 <-  data.frame(
    ...
    person,
    item = paste0("item_",i),
    PIdiff = person - mean(colMeans(fit6.mcmc)[paste0("tau[",i,",",1:4,"]")]),
    lrt = lrt[,i]
  )
...
p
ggsave(plot=p, filename = "figs/model_6_PIdiff_RT.pdf", width = 7, heigh=5, units="in")
