# TEST Dichotomous simulation condition
#run_model_par
...
call_fx <- function(con)
{
  ...
  jags.params <- c(
    "tau", "lambda", "theta",
    "rho",  "icept","prec", "prec.s", "sigma.ts",
    "reli.omega"
  )
  ...
  model.fit <-  R2jags::jags(
    model=paste0(getwd(),"/data/padgettn/diss_sim_results/model_code/dichotomous_items_cond_01_06.txt")
  ...
  out.dir <- paste0(getwd(),"/data/padgettn/diss_sim_results/cond_",c,"/")
...
# Set up Condition 1
CON <- rep(list(list(c=1,i=1, fx_sim_dat=simulate_data_misclass, N_persons=N_persons, N_items=N_items, N_cat = N_cat, tau=sim_tau)), N_rep)
...
registerDoParallel(2) # 8 proc. x 2 nodes
# run model
system.time({
  RESULTS2 <- foreach(
...
for(c in 1:(N_cat-1)){
  ...
  if(c > 1){
    sim_tau[,c] <- sim_tau[,c-1] + runif(N_items, 0.25, 1)
  }
}
