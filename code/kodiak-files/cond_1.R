...
# Set up Condition 1
CON <- rep(list(list(c=1,i=1, fx_sim_dat=simulate_data_misclass, N=N_persons, J=N_items)), N_rep)
...
registerDoParallel(15) # 8 proc. x 2 nodes
# run model
system.time({
  RESULTS2 <- foreach(
...
suppressWarnings(write.csv(RESULTS2, "/data/padgettn/diss_sim_results/cond_1/run_summary_cond_1.csv", col.names = T, row.names = F))
