...
sim_tau <- matrix(
  ...
  ncol=N_cat-1, nrow=N_items
)


simulate_data_misclass <- function(paravec, tau=NULL, use.misclass=TRUE, seed=1){
  set.seed(seed)
  # NOTE: tau is a matrix[J, C-1] of item threshold parameters that possibly vary over items
  # useful functions
  invlogit <- function(x) {exp(x)/(1+exp(x))}
  logit <- function(x){log(x/(1-x))}
  categorizeLRV <- function(x, t){
    y <- 1
    for(c in 1:length(t)){
      if(x > t[c]) y = y + 1
    }
  ...
  categorizePROB <- function(x){
    which(x==max(x))
  }
  getOmega <- function(lambda){
    theta <- 1-lambda**2 # residual variance of y* = 1
  ...
  Y <- Ystar <- Ysampled <- matrix(nrow=N, ncol=J)
  i <- j <- c <- 1
  for(i in 1:N){
    for(j in 1:J){
      ...
      for(c in 2:C){
        ...
        pi.gte[i,j,c] <- invlogit(1.7*(ystar[j,i]-tau[j,(c-1)]))
      }
      ...
      if(use.misclass==F){
        for(c in 1:C){
          omega[i,j,c] = pi[i,j,c]
        }
      }
      # create "categorized" y
      Ystar[i,j] <- categorizeLRV(ystar[j,i], c(tau[j,]))
      Y[i,j] <- categorizePROB(omega[i,j,])
      Ysampled[i,j] <- sample(1:C, 1, prob=omega[i,j,])
      # rescale to 0/1 if dichotomous items
      if(C == 2){
        Y[i,j] = Y[i,j]-1
        Ystar[i,j] = Ystar[i,j]-1
      }
    }
  }
  # true_values <- list(eta0, eta1, lambda, nu, sigma.ei, tau, mulogt, ystar, theta, gamma, omega)
  # names(true_values) <- c("eta", "")
  colnames(Y) <- colnames(Ystar) <- colnames(Ysampled) <- paste0("y", 1:J)
  colnames(logt) <- paste0("logt",1:J)
  sim_data <- list(Y, logt, eta,  ystar, gamma, pi, omega, Ystar, Ysampled, getOmega(lambda))
  names(sim_data) <- c("y", "logt", "eta", "ystar", "gamma", "pi", "omega", "Ystar", "Ysampled", "reliability")
  return(sim_data)
